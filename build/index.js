...
async function makeUSDARequest(url) {
    try {
...
server.tool("get-food-mood-effects", "Get mood effects of a specific food based on its nutritional content", {
    ...
    // Test USDA API call
    const searchUrl = `https://api.nal.usda.gov/fdc/v1/foods/search?query=${encodeURIComponent(food)}&pageSize=1`;
    const searchData = await makeUSDARequest(searchUrl);
    if (!searchData) {
        return {
            content: [
                {
                    type: "text",
                    text: `Failed to retrieve data for ${food}`,
                },
            ],
        };
    }
    return {
        content: [
            {
                type: "text",
                text: `Testing USDA API for ${food}:\n\n${JSON.stringify(searchData, null, 2)}`,
            },
        ],
    };
});
